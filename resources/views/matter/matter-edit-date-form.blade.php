...
<div class="col-sm-9">
	<div class="card">
	  ...
	  <?php //dd($errors);?>
		...
		@endif
	  <div class="card-body">
	    <form method="post" action="{{route('matter-update-date')}}">
			@csrf
            <input type="hidden" name="id" value="{{$caseDate->id}}">
			<div class="form-group">
				...
				@foreach($matters as $matter)
					<option value="{{ $matter->id }}" @if($caseDate->matter_id == $matter->id ) {{"selected"}} @endif>{{ $matter->clients()->first() ? $matter->clients()->first()->client_name : "" }} {{ $matter->case_type()->first()->name }} {{ $matter->case_number}} / {{ $matter->year}}</option>
				@endforeach
				</select>
			...
			<div class="form-group">
			    ...
			    <input type="date" name="next_date" id="inputCaseNextDate"  class="form-control" required="required" placeholder="Case Next Date" value="{{ $caseDate->next_date->format('Y-m-d') }}">
			...
			<div class="form-group">
			    ...
			    <input type="text" name="description" id="inputCaseDescription"  class="form-control" required="required" placeholder="Case description" value="{{ $caseDate->description }}">
			...
			<div class="form-group">
			    ...
			    <input type="text" name="crno" id="inputCRno"  class="form-control" placeholder="Case crno" value="{{ $caseDate->crno }}">
			    @if ($errors->has('crno'))
			...
			<div class="form-group">
			    ...
			    <input type="text" name="srno" id="inputSRno"  class="form-control" placeholder="Case Srno" value="{{ $caseDate->srno }}">
			    @if ($errors->has('srno'))
			...
			<div class="form-group">
			    ...
			    <input type="text" name="remark" id="inputCaseRemark"  class="form-control" placeholder="Case remark" value="{{ $caseDate->remark }}">
